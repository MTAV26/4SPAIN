...
dir_out2 = './4SPAIN/results/observation/cor/'
...
for (isc in 1:length(time_scale)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    postscript(
      file.path(dir_out2,paste("COR_ESP_spi",sc,"_",sprintf("%02d", mesi_8[1]),"_",dataset,"_original.eps",sep = "")),
      paper = "special",
      width = 11,
      height = 7,
      horizontal = T
    ...
    tit <-
      paste(
        'COR (', month.name[ start_date], "= ", (round(mean(corre, na.rm=TRUE),2)),");\n ", dataset, ' against AEMET', sep=""
      )
    ...
    save(corre, file = file.path(dir_out2, paste("COR_ESP_spi",sc,"_",sprintf("%02d", mesi_8[1]),"_",dataset,"_original.RData", sep = "") ))
    save(pvalue_adj, file = file.path(dir_out2, paste("PVALUE_ESP_spi",sc, "_",sprintf("%02d", mesi_8[1]),"_",dataset,"_original.RData", sep = "") ))
