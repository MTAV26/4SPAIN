...
for (isc in 1:length(time_scale)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    for (istart_date in 1:length(start_dates)) {
      ...
      ## plot 1
      postscript(
        file.path(dir_out2,paste("BIAS_ESP_spi",sc,"_",sprintf("%02d", mesi_8[1]),"_",  
      ...
      tit <-
        paste(
          'BIAS for SPI6 in ', dataset, ' (ESP) against AEMET \n Start date: ',  
      ...
      save(corre, file = file.path(dir_out2, paste("BIAS_ESP_spi",sc,"_",sprintf("%02d", mesi_8[1]),
