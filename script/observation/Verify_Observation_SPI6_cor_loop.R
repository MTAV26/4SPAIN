...
dir_out2 = '~/SPAIN2017/results/observation/cor/'
...
for (isc in 1:length(time_scale)) {
  ...
  for (idata in 1:length(datasets)) {
    ...
    #   start_date = start_dates[istart_date]
      ...
      dif_corre= corre_det - corre
      
      
      ## plot 1

      # postscript(
      #   file.path(dir_out2,paste("COR_ESP_spi",sc,"_",dataset,"_original.eps",sep = "")),
      #   paper = "special",
      #   width = 11,
      #   height = 7,
      #   horizontal = T
      ...
      tit <-
        paste(
          'Correlation for SPI6 in ', dataset, ' against AEMET', sep=""
        )
      ...
      # save(corre, file = file.path(dir_out2, paste("COR_ESP_spi",sc,"_",dataset,"_original.RData", sep = "") ))
      # save(pvalue_adj, file = file.path(dir_out2, paste("PVALUE_ESP_spi",sc, "_",dataset,"_original.RData", sep = "") ))
      ...
      # postscript(
      #   file.path(dir_out2,paste("COR_ESP_spi",sc,"_",dataset,"_detrended.eps",sep = "")),
      #   paper = "special",
      #   width = 11,
      #   height = 7,
      #   horizontal = T
      ...
      # save(corre_det, file = file.path(dir_out2, paste("COR_ESP_spi",sc,"_",dataset,"_detrended.RData", sep = "") ))
      # save(pvalue_adj_det, file = file.path(dir_out2, paste("PVALUE_ESP_spi",sc,"_",dataset,"_detrended.RData", sep = "") ))
       dev.off()
      ...
      col_pale <- colorRampPalette(c("dark green","green","white","white","red", "dark red"))(10)
      minC=-0.25
      maxC=0.25
      brk_pale <- seq(minC,maxC, 0.05)
      brk2 = union(-1e+05, brk_pale)
      ...
      postscript(
        file.path(dir_out2,paste("COR_ESP_spi",sc,"_",dataset,"_DIF.eps",sep = "")),
        paper = "special",
        width = 11,
        height = 7,
        horizontal = T
      ...
      tit <-
        paste(
          'Correlation detrended - original (',dataset, ' ;ESP) against AEMET', sep="")
      ...
      #dev.off()
      mioplot(dif_corre, lon,lat,
              toptitle = '',sizetit = 0.8,
              brks = brk_pale,  cols = col_pale,
              axelab =F, filled.continents = FALSE,
              drawleg = F
              # dots = pvalue_det <= 0.05,
              # dots2 = pvalue_adj_det <= 0.05 
      ...
      save(dif_corre, file = file.path(dir_out2, paste("COR_ESP_spi",sc,"_",dataset,"_DIF.RData", sep = "") ))
       dev.off()
